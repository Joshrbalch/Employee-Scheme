...
(define (perform filename . args)
  ...
  (let ((op "ge")  ; Default value for op
        ...
        (function '())) ; Variable to store the function
    (if (not (null? args))
        (if (> (length args) 0)
            (begin
              (set! function (car args)) ; Read the function from args
              (set! args (cdr args))     ; Remove the function from args
              (if (= (length args) 3)
                  (begin
                    (if (string? (car args))
                        (set! op (car args)))
                    (if (string? (car args))
                        (set! threshold (string->number (car args)))))))))
    (set! empList (read-file filename empList))
    (cond
      ((not (member function '("count" "print" "min" "max" "total" "avg")))
      ...
      ((and (not (null? args)) (not (member op '("eq" "ne" "gt" "ge" "lt" "le"))))
       (display "Usage: (perform employee_file action)\nor\nUsage: (perform employee_file action operator threshold)\n\nValid actions: count print min max total avg\nValid operators: eq ne gt ge lt le\n\n"))
      ...
      ((and (string=? function "max") (string=? op "lt"))
       (display-employees (list (getMaxLessThan empList threshold))))
      ((and (string=? function "print") (string=? op "ge"))
       (display-employees (filter (lambda (x) (>= x threshold)) empList)))
      (else #f))))
